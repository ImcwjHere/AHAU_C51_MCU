...
;------------------------------------------------------------
;	.\src\utils\DS18B20\DS18B20.c:7: unsigned char initializeDS18B20(void) {
;	-----------------------------------------
...
_initializeDS18B20:
	...
	ar1 = 0x01
	ar0 = 0x00
;	.\src\utils\DS18B20\DS18B20.c:10: DSPORT = 0;	//将总线拉低480us~960us
;	assignBit
	clr	_P15
;	.\src\utils\DS18B20\DS18B20.c:11: delayTenMicroseconds(64);	//延时642us
	mov	dptr,#0x0040
	lcall	_delayTenMicroseconds
;	.\src\utils\DS18B20\DS18B20.c:13: DSPORT = 1;	//然后拉高总线，如果DS18B20做出反应会将在15us~60us后总线拉低
;	assignBit
	setb	_P15
;	.\src\utils\DS18B20\DS18B20.c:15: while(DSPORT) {	//等待DS18B20拉低总线
	mov	r6,#0x00
	mov	r7,#0x00
00103$:
	jnb	_P15,00105$
;	.\src\utils\DS18B20\DS18B20.c:16: i ++;
	inc	r6
	cjne	r6,#0x00,00121$
	inc	r7
00121$:
;	.\src\utils\DS18B20\DS18B20.c:17: delayOneMillisecond(1);
	mov	dptr,#0x0001
	push	ar7
	push	ar6
	...
	pop	ar6
	pop	ar7
;	.\src\utils\DS18B20\DS18B20.c:18: if(i > 5) return 0;	//等待>5MS, 则初始化失败
	clr	c
	mov	a,#0x05
	subb	a,r6
	mov	a,#(0x00 ^ 0x80)
	...
	ret
00105$:
;	.\src\utils\DS18B20\DS18B20.c:21: return 1;	//初始化成功
	mov	dpl,#0x01
;	.\src\utils\DS18B20\DS18B20.c:22: }
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;	.\src\utils\DS18B20\DS18B20.c:25: void writeByteToDS18B20(unsigned char dat) {
;	-----------------------------------------
;	 function writeByteToDS18B20
;	-----------------------------------------
_writeByteToDS18B20:
	mov	r7,dpl
;	.\src\utils\DS18B20\DS18B20.c:28: for(j = 0;j < 8;j ++) {
	mov	r5,#0x00
	mov	r6,#0x00
00105$:
;	.\src\utils\DS18B20\DS18B20.c:29: DSPORT = 0;	//每写入一位数据之前先把总线拉低1us
;	assignBit
	clr	_P15
;	.\src\utils\DS18B20\DS18B20.c:31: DSPORT = dat & 0x01;	//然后写入一个数据，从最低位开始
	mov	a,r7
	anl	a,#0x01
;	assignBit
	add	a,#0xff
	mov	_P15,c
;	.\src\utils\DS18B20\DS18B20.c:34: while(i --);	//延时68us，持续时间最少60us
	mov	r3,#0x06
	mov	r4,#0x00
00101$:
	mov	ar1,r3
...
00123$:
	...
	orl	a,r2
	jnz	00101$
;	.\src\utils\DS18B20\DS18B20.c:36: DSPORT = 1;	//然后释放总线，至少1us给总线恢复时间才能接着写入第二个数值
;	assignBit
	setb	_P15
;	.\src\utils\DS18B20\DS18B20.c:37: dat >>= 1;
	mov	a,r7
	clr	c
	rrc	a
	mov	r7,a
...
00125$:
	...
	subb	a,#0x00
	jc	00105$
;	.\src\utils\DS18B20\DS18B20.c:39: }
	ret
;------------------------------------------------------------
...
;j                         Allocated to registers r5 r6 
;------------------------------------------------------------
;	.\src\utils\DS18B20\DS18B20.c:42: unsigned char readByteFromDS18B20(void) {
;	-----------------------------------------
;	 function readByteFromDS18B20
;	-----------------------------------------
_readByteFromDS18B20:
;	.\src\utils\DS18B20\DS18B20.c:43: unsigned char byte=0, bi=0;
	mov	r7,#0x00
;	.\src\utils\DS18B20\DS18B20.c:46: for(j = 8;j > 0;j --) {
	mov	r5,#0x08
	mov	r6,#0x00
00105$:
;	.\src\utils\DS18B20\DS18B20.c:47: DSPORT = 0;	//先将总线拉低1us
;	assignBit
	clr	_P15
;	.\src\utils\DS18B20\DS18B20.c:49: DSPORT = 1;	//然后释放总线
;	assignBit
	setb	_P15
;	.\src\utils\DS18B20\DS18B20.c:52: bi = DSPORT;	//读取数据，从最低位开始读取
	mov	c,_P15
	clr	a
	rlc	a
	mov	r4,a
;	.\src\utils\DS18B20\DS18B20.c:53: byte = (byte >> 1) | (bi << 7);	// 将byte左移一位，然后与上右移7位后的bi，注意移动之后移掉那位补0
	mov	a,r7
	clr	c
	rrc	a
	mov	r3,a
	...
	orl	a,r3
	mov	r7,a
;	.\src\utils\DS18B20\DS18B20.c:55: while(i --);
	mov	r3,#0x04
	mov	r4,#0x00
00101$:
	mov	ar1,r3
...
00131$:
	...
	orl	a,r6
	jnz	00105$
;	.\src\utils\DS18B20\DS18B20.c:58: return byte;
	mov	dpl,r7
;	.\src\utils\DS18B20\DS18B20.c:59: }
	ret
;------------------------------------------------------------
;Allocation info for local variables in function 'changeTemperatureCommandOfDS18B20'
;------------------------------------------------------------
;	.\src\utils\DS18B20\DS18B20.c:62: void changeTemperatureCommandOfDS18B20(void) {
;	-----------------------------------------
;	 function changeTemperatureCommandOfDS18B20
;	-----------------------------------------
_changeTemperatureCommandOfDS18B20:
;	.\src\utils\DS18B20\DS18B20.c:63: initializeDS18B20();
	lcall	_initializeDS18B20
;	.\src\utils\DS18B20\DS18B20.c:64: delayOneMillisecond(1);
	mov	dptr,#0x0001
	lcall	_delayOneMillisecond
;	.\src\utils\DS18B20\DS18B20.c:65: writeByteToDS18B20(0xcc);	//跳过ROM操作命令		 
	mov	dpl,#0xcc
	lcall	_writeByteToDS18B20
;	.\src\utils\DS18B20\DS18B20.c:66: writeByteToDS18B20(0x44);	//温度转换命令
	mov	dpl,#0x44
;	.\src\utils\DS18B20\DS18B20.c:67: }
	ljmp	_writeByteToDS18B20
;------------------------------------------------------------
;Allocation info for local variables in function 'readTemperatureCommandOfDS18B20'
;------------------------------------------------------------
;	.\src\utils\DS18B20\DS18B20.c:70: void readTemperatureCommandOfDS18B20(void)
;	-----------------------------------------
;	 function readTemperatureCommandOfDS18B20
;	-----------------------------------------
_readTemperatureCommandOfDS18B20:
;	.\src\utils\DS18B20\DS18B20.c:72: initializeDS18B20();
	lcall	_initializeDS18B20
;	.\src\utils\DS18B20\DS18B20.c:73: delayOneMillisecond(1);
	mov	dptr,#0x0001
	lcall	_delayOneMillisecond
;	.\src\utils\DS18B20\DS18B20.c:74: writeByteToDS18B20(0xCC);	//跳过ROM操作命令
	mov	dpl,#0xcc
	lcall	_writeByteToDS18B20
;	.\src\utils\DS18B20\DS18B20.c:75: writeByteToDS18B20(0xBE);	//发送读取温度命令
	mov	dpl,#0xbe
;	.\src\utils\DS18B20\DS18B20.c:76: }
	ljmp	_writeByteToDS18B20
;------------------------------------------------------------
...
;------------------------------------------------------------
;	.\src\utils\DS18B20\DS18B20.c:80: int readTemperatureOfDS18B20(void) {
;	-----------------------------------------
;	 function readTemperatureOfDS18B20
;	-----------------------------------------
_readTemperatureOfDS18B20:
;	.\src\utils\DS18B20\DS18B20.c:84: changeTemperatureCommandOfDS18B20();	//先写入转换命令
	lcall	_changeTemperatureCommandOfDS18B20
;	.\src\utils\DS18B20\DS18B20.c:85: readTemperatureCommandOfDS18B20();	//然后等待转换完后发送读取温度命令
	lcall	_readTemperatureCommandOfDS18B20
;	.\src\utils\DS18B20\DS18B20.c:86: tml = readByteFromDS18B20();	//读取温度值共16位，先读低字节
	lcall	_readByteFromDS18B20
	mov	r7,dpl
;	.\src\utils\DS18B20\DS18B20.c:87: tmh = readByteFromDS18B20();	//再读高字节
	push	ar7
	lcall	_readByteFromDS18B20
	mov	r6,dpl
	pop	ar7
;	.\src\utils\DS18B20\DS18B20.c:88: temp = tmh;
;	.\src\utils\DS18B20\DS18B20.c:89: temp <<= 8;
	mov	ar5,r6
;	.\src\utils\DS18B20\DS18B20.c:90: temp |= tml;
	clr	a
	mov	r6,a
	mov	r4,a
	mov	a,r7
	...
	orl	a,r5
	mov	dph,a
;	.\src\utils\DS18B20\DS18B20.c:92: return temp;
;	.\src\utils\DS18B20\DS18B20.c:93: }
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;	.\src\utils\DS18B20\DS18B20.c:95: void getTemperatureOfDS18B20(unsigned char * buf, float * temperature) {
;	-----------------------------------------
...
;	.\src\utils\DS18B20\DS18B20.c:98: temp = readTemperatureOfDS18B20();
	lcall	_readTemperatureOfDS18B20
	mov	r3,dpl
;	.\src\utils\DS18B20\DS18B20.c:99: if (temp < 0) {    // 当温度值为负数
	mov	a,dph
	mov	r4,a
	jnb	acc.7,00102$
;	.\src\utils\DS18B20\DS18B20.c:100: buf[0] = '-';    //   -
	...
	mov	a,#0x2d
	lcall	__gptrput
;	.\src\utils\DS18B20\DS18B20.c:102: temp = temp - 1;
	mov	a,r3
	add	a,#0xff
	mov	r1,a
	mov	a,r4
	addc	a,#0xff
	mov	r2,a
;	.\src\utils\DS18B20\DS18B20.c:103: temp = ~temp;
	mov	a,r1
	cpl	a
	mov	dpl,a
	mov	a,r2
	cpl	a
	mov	dph,a
;	.\src\utils\DS18B20\DS18B20.c:104: tp = temp;
	lcall	___sint2fs
	mov	r0,dpl
	mov	r1,dph
	mov	r2,b
	mov	r7,a
;	.\src\utils\DS18B20\DS18B20.c:105: temp = tp * 0.0625 * 100 + 0.5;
	push	ar0
	push	ar1
	push	ar2
	push	ar7
	...
	ljmp	00103$
00102$:
;	.\src\utils\DS18B20\DS18B20.c:110: tp = temp;  //因为数据处理有小数点所以将温度赋给一个浮点型变量
	mov	dpl,r3
	mov	dph,r4
	lcall	___sint2fs
	mov	r2,dpl
	...
	mov	r4,b
	mov	r5,a
;	.\src\utils\DS18B20\DS18B20.c:112: temp = tp * 0.0625 * 100 + 0.5;
	push	ar2
	push	ar3
	push	ar4
	push	ar5
	...
	mov	a,r5
	lcall	___fs2sint
;	.\src\utils\DS18B20\DS18B20.c:116: buf[0] = (temp / 10000) + '0';
	mov	r6,dpl
	mov	r7,dph
	...
	lcall	__gptrput
00103$:
;	.\src\utils\DS18B20\DS18B20.c:118: buf[1] = (temp % 10000 / 1000) + '0';
	mov	a,#0x01
	...
	lcall	__gptrput
;	.\src\utils\DS18B20\DS18B20.c:119: buf[2] = (temp % 1000 / 100) + '0';
	mov	a,#0x02
	add	a,_getTemperatureOfDS18B20_buf_65536_28
	...
	lcall	__gptrput
;	.\src\utils\DS18B20\DS18B20.c:120: buf[3] = '.';
	mov	a,#0x03
	add	a,_getTemperatureOfDS18B20_buf_65536_28
	mov	r0,a
	clr	a
	...
	mov	a,#0x2e
	lcall	__gptrput
;	.\src\utils\DS18B20\DS18B20.c:121: buf[4] = (temp % 100 / 10) + '0';
	mov	a,#0x04
	add	a,_getTemperatureOfDS18B20_buf_65536_28
	...
	lcall	__gptrput
;	.\src\utils\DS18B20\DS18B20.c:122: buf[5] = 0xDF;
	mov	a,#0x05
	add	a,_getTemperatureOfDS18B20_buf_65536_28
	mov	r2,a
	clr	a
	...
	mov	a,#0xdf
	lcall	__gptrput
;	.\src\utils\DS18B20\DS18B20.c:123: buf[6] = 'C';
	mov	a,#0x06
	add	a,_getTemperatureOfDS18B20_buf_65536_28
	mov	r2,a
	clr	a
	...
	mov	a,#0x43
	lcall	__gptrput
;	.\src\utils\DS18B20\DS18B20.c:126: if (buf[0] == '-') {
	mov	dpl,_getTemperatureOfDS18B20_buf_65536_28
	mov	dph,(_getTemperatureOfDS18B20_buf_65536_28 + 1)
	mov	b,(_getTemperatureOfDS18B20_buf_65536_28 + 2)
	lcall	__gptrget
...
00141$:
	ljmp	00105$
00142$:
;	.\src\utils\DS18B20\DS18B20.c:127: tp = (float)(-1 * ((int)(buf[1] - '0') * 10 + (int)(buf[2] - '0') + (int)(buf[4] - '0') * 0.1));
	mov	dpl,_getTemperatureOfDS18B20_sloc4_1_0
	mov	dph,(_getTemperatureOfDS18B20_sloc4_1_0 + 1)
	mov	b,(_getTemperatureOfDS18B20_sloc4_1_0 + 2)
	lcall	__gptrget
	...
	ljmp	00106$
00105$:
;	.\src\utils\DS18B20\DS18B20.c:129: tp = (float)((int)(buf[0] - '0') * 100 + (int)(buf[1] - '0') * 10 + (int)(buf[2] - '0') + (int)(buf[4] - '0') * 0.1);
	mov	dpl,_getTemperatureOfDS18B20_buf_65536_28
	mov	dph,(_getTemperatureOfDS18B20_buf_65536_28 + 1)
	mov	b,(_getTemperatureOfDS18B20_buf_65536_28 + 2)
	lcall	__gptrget
	...
	mov	sp,a
00106$:
;	.\src\utils\DS18B20\DS18B20.c:131: *temperature = tp;
	...
	mov	a,r7
	lcall	__gptrput
;	.\src\utils\DS18B20\DS18B20.c:134: if (buf[0] == '0') buf[0] = ' ';
	mov	dpl,_getTemperatureOfDS18B20_buf_65536_28
	mov	dph,(_getTemperatureOfDS18B20_buf_65536_28 + 1)
	mov	b,(_getTemperatureOfDS18B20_buf_65536_28 + 2)
	lcall	__gptrget
	...
	lcall	__gptrput
00108$:
;	.\src\utils\DS18B20\DS18B20.c:136: if (buf[1] == '0' && (buf[0] == ' '||buf[0] == '-')) {
	mov	dpl,_getTemperatureOfDS18B20_sloc4_1_0
	mov	dph,(_getTemperatureOfDS18B20_sloc4_1_0 + 1)
	mov	b,(_getTemperatureOfDS18B20_sloc4_1_0 + 2)
	lcall	__gptrget
...
00147$:
	...
	cjne	r7,#0x2d,00113$
00109$:
;	.\src\utils\DS18B20\DS18B20.c:137: buf[1] = buf[0];
	mov	dpl,_getTemperatureOfDS18B20_buf_65536_28
	mov	dph,(_getTemperatureOfDS18B20_buf_65536_28 + 1)
	mov	b,(_getTemperatureOfDS18B20_buf_65536_28 + 2)
	lcall	__gptrget
	...
	mov	b,(_getTemperatureOfDS18B20_sloc4_1_0 + 2)
	lcall	__gptrput
;	.\src\utils\DS18B20\DS18B20.c:138: buf[0] = ' ';
	mov	dpl,_getTemperatureOfDS18B20_buf_65536_28
	mov	dph,(_getTemperatureOfDS18B20_buf_65536_28 + 1)
	mov	b,(_getTemperatureOfDS18B20_buf_65536_28 + 2)
	mov	a,#0x20
;	.\src\utils\DS18B20\DS18B20.c:141: }
	ljmp	__gptrput
00113$:
	ret
	.area CSEG    (CODE)
